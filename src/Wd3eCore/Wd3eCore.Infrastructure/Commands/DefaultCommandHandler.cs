...
namespace Wd3eCore.Environment.Commands
{
    public abstract class DefaultCommandHandler : ICommandHandler
    {
        ...
        private void SetSwitchValue(KeyValuePair<string, string> commandSwitch)
        {
            ...
            if (!propertyInfo.GetCustomAttributes(typeof(Wd3eSwitchAttribute), false).Any())
            {
                throw new InvalidOperationException(S["A property \"{0}\" exists but is not decorated with \"{1}\"", commandSwitch.Key, typeof(Wd3eSwitchAttribute).Name]);
            }
        ...
        private void CheckMethodForSwitches(MethodInfo methodInfo, IDictionary<string, string> switches)
        {
            ...
            foreach (Wd3eSwitchesAttribute switchesAttribute in methodInfo.GetCustomAttributes(typeof(Wd3eSwitchesAttribute), false))
            {
