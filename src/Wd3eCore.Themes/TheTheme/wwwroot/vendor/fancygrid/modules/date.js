  ...
  F.define(['Fancy.form.field.Date', 'Fancy.DateField'], {
    ...
    showPicker: function () {
      ...
      var offset = input.offset(),
        //x = offset.left;
        x = offset.left - 308 + input.width() + 16,
    ...
    onInput: function () {
      var me = this,
        ...
        //lp加  2018年10月22日16:10:53
        var re = /^(\d{1,2})-(\d{1,2})$/;
        var re2 = /^(\d{1})-(\d{1})$/;
        var re3 = /^(\d{1,2})-(\d{1})$/;
        var re4 = /^(\d{1})-(\d{1,2})$/;
        var date22 = new Date(this.date);
        var year22 = date22.getFullYear();   //当前时间 年份

        if (F.Date.parse(value, me.format.edit, me.format.mode).toString() === 'Invalid Date') {

            //lp加  2018年10月22日16:11:43
            if (re.test(value) && (value.length == 5)) {//判断日期格式符合YYYY-MM-DD标准
                value = year22 + "-" + value;
            } else if (re2.test(value)) {
                var tela = value.substr(0, 1); //前
                var telb = value.substr(value.length - 1, 1);  //后
                value = year22 + "-0" + tela + "-0" + telb;
            } else if (re3.test(value)) {
                var tela = value.substr(0, 3); //前
                var telb = value.substr(value.length - 1, 1);  //后
                value = year22 + "-" + tela + "-0" + telb;
            } else if (re4.test(value)) {
                value = year22 + "-0" + value;
            } else {
                return;
            }
      }
